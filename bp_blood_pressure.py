...
def Test():
    ...
    n = len(data)
    train_data = data[0:int(0.8*n), 0:-2]
    train_label = data[0:int(0.8*n), -2]
    train_label1 = data[0:int(0.8*n), -1]
    test_data = data[int(0.8*n):, 0:-2]
    test_label = data[int(0.8*n):, -2]
    test_label1 = data[int(0.8*n):, -1]
    ...
    for i in range(0,len(test_label1)):
        ...
        err = err + abs((test_label1[i] - out_act[0])*(dbpmax - dbpmin))
        if abs((test_label1[i] - out_act[0])*(dbpmax - dbpmin)) > 10:
            t10 = t10 + 1
    print err/tn, t10*1.0/tn
